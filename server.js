...
app.post("/scrape", async (req, res) => {
  ...
  try {
    ...
    const browser = await puppeteer.launch({
      headless: 'new',  // Use 'new' for Puppeteer 23+ or 'true' for legacy
      args: [
        "--no-sandbox",
        "--disable-setuid-sandbox",
        "--disable-dev-shm-usage",
        "--disable-gpu",
        "--disable-accelerated-2d-canvas",  // Additional stability
        "--disable-software-rasterizer",    // Additional stability
        "--disable-dev-shm-usage",          // Already present, added again for clarity
        "--disable-extensions",             // Disable extensions for stability
        "--no-zygote",
        "--single-process",
        "--shm-size=1gb",                   // Increase shared memory to avoid out-of-memory errors
      ],
    });
    ...
    const data = await page.evaluate(() => {
      ...
      const languages = Array.from(document.querySelectorAll('.languages ul > li')).map(language => {
        ...
        const languageDescription = language.querySelector('h4') ? language.querySelector('h4').textContent.trim() : '';
